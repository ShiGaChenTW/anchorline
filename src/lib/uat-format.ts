...
**附件：**
- ![T1-01](uat-assets/<報告檔名>/T1-01.png) 紅框是實際位置

## 補充說明

1. 題目沒問到、但這輪測到的問題或建議
   ![S1-01](uat-assets/<報告檔名>/S1-01.png) 這張圖要說明什麼
\`\`\`
...
- \`**附件：**\` 是該題的截圖。使用者用「貼上截圖」（剪貼簿）或「選檔」加入，可多張。
  檔名是 \`T題號-流水.ext\`，圖檔在 \`plans/uat-assets/<報告檔名>/\`。
  **出題時不要預填附件，也不要手寫圖片。**
- \`## 補充說明\` 是報告末的編號列表，給題目沒覆蓋到的問題或優化建議。
  每則可帶圖（檔名 \`S編號-流水.ext\`）。**這不是一題**，沒有錨點，不計入進度。
  **出題時不要發明補充說明，也不要寫 \`## 補充\` 當題目標題**——那個標題會被解析成補充區，後面的題會消失。
...
若該題有 \`**附件：**\`，**先 Read 那些圖檔**再重現，不要只靠文字想像畫面。

報告末若有 \`## 補充說明\`，那是使用者另外記下的問題或建議，**不是帶錨點的失敗題**。
先讀文字與圖，再判斷要不要另開工作；不要為補充項偽造 \`anc:t=\`。
