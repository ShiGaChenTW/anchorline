...
| 不做 | 為什麼 |
|---|---|
| **App 內做 Kanban 任務 CRUD / 拖拉** | Backlog.md、Linear、GitHub Projects 已解；且會與你每天實際用的 Claude Code / Orca 競爭並輸掉 |
| **把事件流放 localStorage** | 5–10MB 硬上限、不可 diff、不可稽核、重灌即消失 |
| **自建 `openspec/specs/*.md` parser** | 2026-08-08 報告已定案：上游格式會演進，會變成維護第二套 parser |
