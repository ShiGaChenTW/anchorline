...
> 想多做半天就加 **Phase 1.7 跨 repo PR 雷達**（§十一）——`gh` 已裝已 auth，而實測時發現有一個 PR 開了 38 天沒動、完全不在視野裡。
>
...
第二個結論：**「開發 LOG 資料庫」的正確名字是「AI 開發稽核軌跡」。** 那才是差異化所在，而稽核軌跡正是市場上每一個 AI 工作台都沒有處理的那一塊。
...
- **作品化**：把上面那份 replay 做成公開可看的案例頁。這是整條治理鏈唯一能對外展示的產物，也是「為什麼要做治理鏈」最短的說明方式
...
| **一次做完三階段** | 每個階段各自可出貨，一次做完等於把三次驗證機會併成一次。Phase 1 自己就是完整產品 |
...
3. **每階段缺「完成的儀式」。** 已有停損條件（何時停手），但沒有 done signal（何時算贏）。兩者都需要——停損定義失敗，儀式定義成功，只有前者的專案會無限延長。建議每階段補一條：Phase 0 = 截一張帶 ID 的 `bun run track` 圖；Phase 1 = 截焦點卡發一則貼文；Phase 2 = 匯出一份稽核報告 PDF。**產出物離開電腦，那一階段才算完成。**
...
| **現在開著的 PR** | 🔴 **1 個，開了 38 天沒動**（1166 行、MERGEABLE、零 review） |
...
2. **PR 追蹤的價值不在單一 repo，在跨 repo。** 只看本 repo 會判定這功能沒用（0 個 PR，直推 main）。放大到同一個帳號的所有 repo，它立刻變成最有價值的一個——實測就抓到一個掛了 38 天的 PR，**完全不在任何畫面上**。
...
> 起因：重新評估 §七 不做清單裡的三項。
> 結論：**一項維持不做，兩項縮小範圍後翻案。** 翻案的理由是新事實，不是被問了第二次。
...
## 十三、「做成軟體，不只是網頁」的評估（2026-08-09 追加）

### 13.1 先釐清：它已經是軟體了

`/Applications/PRD開發監控台 測試.app` 現在正在跑。原生殼 1157 行 Swift、12 個 bridge action、1.3 MB。**瀏覽器版從來就不是產品，是降級路徑**——沒有磁碟、沒有 git、沒有 CLI。

所以真正要決定的不是「要不要做成軟體」，而是**要不要放棄瀏覽器版**。這兩句話的工程後果完全不同。

### 13.2 放棄瀏覽器版拿回什麼

| 現況包袱 | 數量 | 放棄後 |
|---|---|---|
| `isDesktop()` / `canScanPlans()` / `canQueryStatus()` / `canWriteLog()` 分支 | **19 處，散在 11 個檔** | 每個功能只剩一條路徑 |
| `import.meta.glob` 編譯期快照 | **5 處**（plans ×4、log ×1） | 全刪，資料一律即時讀磁碟 |
| §12.3 自建 openspec parser | 待決 | **整項消失**——桌面版永遠有 CLI |
| 焦點卡 `openspecPct` | 硬寫 `null` | 可以真的接上 |
| 測試 surface | 降級路徑也要測 | 少一半 |

**這是一個「減法」的決定。** 少寫程式碼、少一套要維護的行為、少一個「在瀏覽器裡看起來壞掉但其實是設計」的解釋成本。

### 13.3 但會撞到三件事

#### (a) 與「要能對外展示」直接打架 ⚠️ 最重要

這個專案有一個目標是**產出一份可公開的案例研究**。而未簽章的 macOS 專屬 App 是最糟的展示形式——開發者自己都要跑 `xattr -dr com.apple.quarantine` 才打得開。沒有人會為了看一份案例研究安裝一個 1.3 MB 的不明程式。

**解法（而且你已經有一半了）**：**App 是產品，作品是 App 產出的檔案。**

- `replay.ts` → 治理鏈 Markdown
- `log-views.ts` → 稽核報告 Markdown / CSV
- `artifacts/P2-audit-report.pdf` → 3 頁，已經產出來了

這些**不需要安裝任何東西**就能看。所以兩個目標不衝突，但界線要講死：**作品頁不是 App 的一個頁面，是 App 的一個匯出**。這句話一旦確立，「作品頁跑在瀏覽器裡」這個顧慮就整個消失——它不是網頁版 App，是一份靜態檔。
...
意思是**沒有開發者簽章**。在你自己機器上沒問題；給任何第三方都會被 Gatekeeper 擋下「無法驗證開發者」。要解需要 Apple Developer Program（US$99/年）＋ notarization 流程。

**判準很乾脆**：
- 只有你自己用 → **不需要**，現況就夠，這條成本是零
- 要給任何一個別人用 → 這 $99 與流程是硬成本，而且要排進時程

#### (c) 跨平台：WKWebView 綁死 macOS

換 Tauri 或 Electron 才能跨平台，代價是重寫那 1157 行 Swift bridge。

| 方案 | 體積 | 跨平台 | 重寫成本 |
|---|---|---|---|
| **WKWebView（現況）** | 1.3 MB | ❌ macOS only | — |
| Tauri | ~10 MB | ✅ | 重寫 bridge（Rust），約 1–2 週 |
| Electron | ~150 MB | ✅ | 重寫 bridge（Node），約 1 週 |

**建議不要換。** 你只有 mac，換來的跨平台現在沒有使用者，而 1.3 MB → 150 MB 是實實在在的退步。真的要跨平台的那天再說，那時 bridge 契約已經穩定，移植反而更便宜。

### 13.4 三個選項

| 選項 | 內容 | 適合 |
|---|---|---|
| **A. 桌面優先 + 匯出作品**（建議） | 刪 19 個 guard 與 5 個快照，openspec 接真的；作品走匯出檔 | 自己用為主，偶爾要對外展示 |
| B. 雙軌並行（現況） | 兩條路徑都維護 | 你真的預期有人會用瀏覽器版 |
| C. 換 Tauri 跨平台 | 重寫殼 | 要賣給別人、要 Windows 使用者 |

**建議 A。** 理由：B 的成本是永遠兩套行為要驗、每個新功能問一次「瀏覽器版怎麼辦」；C 解的是你現在沒有的問題。

### 13.5 選 A 之後的連鎖影響

1. **§12.3 自建 openspec parser → 取消**。桌面版永遠有 CLI，那項的唯一理由就是瀏覽器瞎眼。省 0.5 天。
2. **焦點卡接上真的 openspec 進度** —— `openspecPct` 不再是 `null`，rollup 兩半都有。
3. **刪 19 個 guard 與 5 個編譯期快照** —— 淨減少程式碼。
4. **`vite.config.ts` 的 `base:"./"` 保留** —— 那是 `file://` 相容，App 內載入靠它，不是為了瀏覽器。
5. 作品匯出獨立成一條路：Markdown / PDF / 單檔 HTML，不依賴 App 執行環境。
...
- 只有自己 → 現況的 adhoc 簽章就夠，直接走 A，零額外成本
- 會給別人（哪怕一個人）→ 要排 Apple Developer Program $99/年 + notarization，那是獨立的一段工作，不該混在功能開發裡

**在你回答之前我採預設：只有你自己用。** 這個預設如果錯了，代價只是之後補做簽章，不會白做任何東西。
