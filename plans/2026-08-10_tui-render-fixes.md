...
**未完成**
- `tracking.html` 計劃列表的瀏覽器驗證（見阻塞，需要桌面版）。
- PRD T2 剩下的裝飾項（gauge 改 ASCII、相對時間、spinner、過小視窗守衛）沒做——
  它們都不是「會說謊」的缺陷，等真的礙眼再說。

**後續建議**
- 先決定網頁端要不要一起收。三個呈現端共用 `planProgress()` 之後就沒有分歧的餘地了。
- T3（雙欄完整版／滑鼠／設定面板）維持不做。
