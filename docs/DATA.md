...
│   └── 2026-09.jsonl
├── context/
│   └── MyProject-20260812-0940.md   # 專案分析報告，AI 撰寫的前置條件
└── exports/
    └── MyProject-20260812-1430.md   # 匯出的 PRD／HTML／備份 JSON
```

**分析報告為什麼是第二種檔。** AI 撰寫需要「這個專案長什麼樣」當背景，
而模型手上只有一個標題時會很流暢地寫出一份跟這個 repo 沒有關係的提案。
分析報告**不覆寫**：檔名帶時間，每次重讀都是新檔 —— 蓋掉就沒有東西可以回答
「這中間變了什麼」。契約見 `BRIDGE.md` §4.7d。

**匯出為什麼也在這。** 桌面殼的 WKWebView 對瀏覽器下載無聲失敗，
所以「匯出 MD」把檔案寫進 `exports/` 並顯示完整路徑。匯出檔可重生，
同名**允許覆寫**（跟分析報告相反）；要不要進 git 由你決定，預設不忽略。
